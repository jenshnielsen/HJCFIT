...
#  endif
  ...
  ExactSurvivor :: RecursionInterface::RecursionInterface( QMatrix const & _qmatrix,
    ...
    Eigen::EigenSolver<t_rimatrix> eigsolver(transitions.matrix);
...
#   endif
    ...
    t_rimatrix const eigenvectors = eigsolver.eigenvectors().real();
    t_rimatrix const eigenvectors_inv = eigsolver.eigenvectors().inverse().real();
    ...
    t_rimatrix const exponential_factor = (_tau * transitions.ff()).exp() * transitions.fa();
    for(t_rvector::Index i(0); i < eigenvalues_.size(); ++i) {
      auto left = eigenvectors.col(i).head(transitions.nopen);
  ...
  namespace {
    ...
    template<class T> 
      typename T::t_element M_m_of_t(T &_C, t_uint _m, t_real _t) {
        //alloc(open close)
        t_rimatrix result = B_im_of_t(_C, 0, _m, _t) * std::exp(-_C.get_eigvals(0)*_t);
    ...
    template<class T> 
      typename T::t_element R_of_t(T &_C, t_real _t, t_real _tau) {
        ...
        t_real current_t(_t);
        // alloc(open,close)
        t_rimatrix result = M_m_of_t(_C, 0, _t);
        t_uint m=1;
  ...
  t_rimatrix ExactSurvivor :: af(t_real _t) const {
    if(_t < 0e0) return recursion_af_->zero();
    return R_of_t(*recursion_af_, _t, tau_);
  }
  t_rimatrix ExactSurvivor :: fa(t_real _t) const {
    if(_t < 0e0) return recursion_fa_->zero();
    return R_of_t(*recursion_fa_, _t, tau_);
  }

  t_rimatrix ExactSurvivor :: recursion_af(t_uint _i, t_uint _m, t_uint _l) const {
  ...
  t_rimatrix ExactSurvivor :: recursion_fa(t_uint _i, t_uint _m, t_uint _l) const {
  ...
  t_rimatrix ExactSurvivor :: D_af(t_uint _i) const {
    if(_i >= recursion_af_->nbeigvals())  
      throw errors::Index("i index should be smaller than the number of eigenvalues.");
    return  recursion_af_->getD(_i);
  }
  t_rimatrix ExactSurvivor :: D_fa(t_uint _i) const {
    if(_i >= recursion_fa_->nbeigvals())  
      throw errors::Index("i index should be smaller than the number of eigenvalues.");
